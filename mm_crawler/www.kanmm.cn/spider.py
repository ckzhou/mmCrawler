...
# -*- coding:utf-8 -*-
# ahthor:zhouzh
# Program Featrue:Grab Grils Pictrues of http://www.kanmm.cn
...
def setcmdargs():
	...
	parser.add_argument("-u",type=str,default="www.kanmm.cn",help=u"指定提供美女图片的网站")
...
def getpage(url):
	"""根据url请求页面并记录相应的日志"""
	times=1
	while(times<4):  # 网络请求失败后最多重复三次
		try:
			page=urllib.urlopen(url)
			pagehtml=page.read()
			pagehtml=pagehtml.decode("gbk").encode("utf-8")
			return pagehtml
		except IOError:
			if(times<3):
				times+=1
...
def invokespider(site,threadnum,storedir,picsnum):
	...
	if(not pageHtml):  # 请求网站首页，如果失败，退出程序
		print u"请求%s失败." %(site)
		logging.warning("请求%s失败",site)
		exit()
	menuhtml=re.findall('<ul class="nav box990 f14" id="nav">[\s\S]*?</ul>',pageHtml)[0]
	categorylinks=re.findall("<a href='(.*?)'><span>(.*?)</span></a>",menuhtml)  # 提取图集分类的锚点
...
def extractalbums(pool,category):
	...
	if(not pagehtml):
		...
		return 
	albumslist=re.findall('<div class="wrapper fix">[\s\S]*</div>',pagehtml)  # 提取包含图集的html
	if(len(albumslist)>0):  # 如果albumslist存在，说明是一个有效的分类页面
		logging.info("开始下载%s的第%d页",categorytext,page)
		albums=re.findall('<a href="(.*?)" title="(.*?)" class="img" target="_blank">',albumslist[0])  # 提取图集的锚点
...
def downloadpic(pool,album,threadata=None):
	...
	picurl=pool.site+re.findall("<img src='(.*?)' alt='.*?' title=\".*?\" id='bigimg' />",pagehtml)[0]
	retrievepic(pool,picurl,filename)
	nextpage=re.findall("<a href='(\d+?_\d+?\.html)'>下一页</a>",pagehtml)  #  提取图集下一页的锚点
